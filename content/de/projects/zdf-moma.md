---
title: ZDF moma
active: true
category:
  - code
  - studio
  - news
  - infographics
coverImage:
  src: /images/projects/zdf-moma/ZDF-moma.jpg
  alt: ZDF moma
detailImage:
  src: /images/Logos/ZDF.svg
  alt: ZDF
header: ZDF
projectsHeader: Unsere Projekte
sliderHeader: Projekt Bilder
sliderItems:
  - type: image
    src: /images/projects/zdf-moma/01-zdf-moma-studio.jpg
    alt: ZDF moma Studio
    title: Studio
  - type: video
    provider: vimeo
    videoId: "465154031"
    title: CapeRock Trailer
    autoPlay: true
    poster: /images/projects/idlePlay.jpg
  - type: image
    src: /images/projects/zdf-moma/02-zdf-moma-studio.jpg
    alt: Studio
    title: ""
  - type: image
    src: /images/projects/zdf-moma/03-zdf-moma-tisch-mod.jpg
    alt: Studio Tisch
    title: ""
  - type: image
    src: /images/projects/zdf-moma/04-zdf-moma-studio.jpg
    alt: Studio
    title: ""
  - type: image
    src: /images/projects/zdf-moma/05-zdf-moma-gaesteliste.jpg
    alt: Gästeliste
    title: ""
  - type: image
    src: /images/projects/zdf-moma/06-zdf-moma-wetter.jpg
    alt: Wetter
    title: ""
  - type: image
    src: /images/projects/zdf-moma/07-zdf-moma-tisch-total.jpg
    alt: Studio Total
    title: ""
  - type: image
    src: /images/projects/zdf-moma/08-zdf-moma-social.jpg
    alt: Studio Social
    title: ""
  - type: image
    src: /images/projects/zdf-moma/11-zdf-moma-social-sofa.jpg
    title: ""
slug: zdf-moma
table:
  header:
    firstCol: Unsere Aufgaben
    secondCol: Technologien
  tasks:
    - Design Integration, Technisches Konzept
    - Videowalls, Grafik Szenen, Templates
  technologies:
    - Viz Artist, Viz Pilot
    - Adobe CC
type: project
---

::project-details
---
:image-src: detailImage
:projects-header: projectsHeader
:slider-header: sliderHeader
:slider-items: sliderItems
:table-details: table
---
ZDF

#project-name
moma

#body
So gut sieht das neue [ZDF Morgenmagazin](https://www.zdf.de/magazine/zdf-morgenmagazin-104){:target="_blank"} "moma" aus! Die Umgestaltung des Studios ist für das ZDF mehr als ein Redesign: Das Fernsehstudio wird zum Raum, in dem man die Arbeit der Journalisten-Teams mit erleben kann.

#sub
Das Design wurde von unserer befreundeten Agentur [CapeRock](https://www.caperock.tv){:target="_blank"} (vielen Dank für das Video!) aus Amsterdam entwickelt. Wieder einmal haben wir in wunderbar kollegialer Weise mit dem ZDF zusammen gearbeitet.
::
